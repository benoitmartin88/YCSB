...
public class TransactionalWorkload extends Workload {
  ...
  public static final String MESSAGE_PROPORTION_PROPERTY = "messageproportion";

  /**
   * The default proportion of transactions that are scans.
   */
  public static final String MESSAGE_PROPORTION_PROPERTY_DEFAULT = "0.0";
  /**
  ...
  public static final String MESSAGE_DESTINATION_COUNT_PROPERTY = "messagedestinationcount";
  public static final String MESSAGE_DESTINATION_COUNT_PROPERTY_DEFAULT = "2";

  protected NumberGenerator keysequence;
  protected DiscreteGenerator operationchooser;
  protected NumberGenerator keychooser;

  protected NumberGenerator destinationchooser;
  ...
  protected long messageDestinationCount;
  ...
  public void init(Properties p) throws WorkloadException {
    ...
    fieldlengthgenerator = TransactionalWorkload.getFieldLengthGenerator(p);
    ...
    // TODO: make variables
    operationsPerTransaction = Integer.parseInt(p.getProperty("operationspertransaction", "1"));
    messageDestinationCount = Long.parseLong(
        p.getProperty(MESSAGE_DESTINATION_COUNT_PROPERTY, MESSAGE_DESTINATION_COUNT_PROPERTY_DEFAULT));

    destinationchooser = new UniformLongGenerator(0, messageDestinationCount - 1);
  }
  ...
  public boolean doInsert(DB db, Object threadstate) {
    ...
    String dbkey = TransactionalWorkload.buildKeyName(keynum, zeropadding, orderedinserts);
  ...
  public boolean doTransaction(DB db, Object threadstate) {
    String tid = doTransactionPrepare(db);
    if(tid.isEmpty()) {
      return false;
    }
    ...
    for(int i=0; i<operationsPerTransaction; ++i) {
      ...
      case "UPDATE":
        ...
        break;
      case "INSERT":
        doTransactionInsert(db, tid);
        break;
      case "SCAN":
        doTransactionScan(db, tid);
        break;
      case "MESSAGE":
        doTransactionMessage(db, tid);

        break;
      default:
        doTransactionReadModifyWrite(db, tid);
      }
    }

    doTransactionCommit(db, tid);
  ...
  long nextMessageDestination() {
    return destinationchooser.nextValue().longValue();
  }


  public String doTransactionPrepare(DB db) {
    return db.prepare();
  }

  public void doTransactionCommit(DB db, String tid) {
    db.commit(tid);
  }

  public void doTransactionAbort(DB db, String tid) {
    db.abort(tid);
  }

  public void doTransactionRead(DB db, String tid) {
    // choose a random key
    long keynum = nextKeynum();

    String keyname = TransactionalWorkload.buildKeyName(keynum, zeropadding, orderedinserts);
    ...
    db.read(table, keyname, fields, cells, tid);
  ...
  public void doTransactionReadModifyWrite(DB db, String tid) {
    // choose a random key
    long keynum = nextKeynum();
    ...
    db.update(table, keyname, values, tid);
  ...
  public void doTransactionScan(DB db, String tid) {
    // choose a random key
    long keynum = nextKeynum();

    String startkeyname = TransactionalWorkload.buildKeyName(keynum, zeropadding, orderedinserts);
  ...
  public void doTransactionInsert(DB db, String tid) {
    // choose the next key
    ...
    try {
      String dbkey = TransactionalWorkload.buildKeyName(keynum, zeropadding, orderedinserts);
      HashMap<String, ByteIterator> values = buildValues(dbkey);
      db.insert(table, dbkey, values, tid);
    } finally {
  ...
  protected static DiscreteGenerator createOperationGenerator(final Properties p) {
    ...
    final double messageproportion = Double.parseDouble(
        p.getProperty(MESSAGE_PROPORTION_PROPERTY, MESSAGE_PROPORTION_PROPERTY_DEFAULT));
    ...
    if (messageproportion > 0) {
      operationchooser.addValue(messageproportion, "MESSAGE");
    }
